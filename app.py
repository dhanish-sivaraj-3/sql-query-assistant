...
def health():
    ...
    try:
        ...
        app.logger.error(f"Gemini connection test failed: {e}")
        gemini_connected = False
...
def get_databases():
    ...
    try:
        ...
        app.logger.error(f"Error getting databases: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
...
def get_tables_with_columns(database):
    ...
    try:
        # Use the detailed method to get tables with column information
        result = db_connector.get_detailed_tables_info(database)
        return jsonify(result)
    except Exception as e:
        app.logger.error(f"Error getting tables for {database}: {str(e)}")
...
def connect_custom_database():
    ...
    try:
        ...
        app.logger.info(f"Custom connection attempt: {server} (Type: {db_type}, DB: {database})")
        ...
        if temp_connector.test_connection():
            ...
            return jsonify({
                "success": False,
                "error": f"Failed to connect to server {server}. Please check credentials and network settings."
            }), 400
            
    except Exception as e:
        app.logger.error(f"Custom connection error: {str(e)}")
        return jsonify({
            "success": False, 
...
def handle_query():
    ...
    try:
        ...
        app.logger.info(f"Processing query for database {database}: {query}")
        ...
        llm_result = gemini_client.generate_sql_query(
            ...
            db_connector,
            database,
            conversation_history.get(f"{session_id}_{database}")
        )
        ...
        # Generate explanation of results using Gemini
        results_summary = {
...
def get_schema(database):
    ...
    try:
        schema_info = db_connector.get_schema_info(database)
        return jsonify(schema_info)
    except Exception as e:
        app.logger.error(f"Error fetching schema for {database}: {str(e)}")
        return jsonify({"success": False, "error": str(e)}), 500
